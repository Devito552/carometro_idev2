...
document.getElementById('cadastroUsuarioForm').addEventListener('submit', async function (event) {
    ...
    const formData = new FormData(document.getElementById('cadastroUsuarioForm'));
    ...
    if (response.ok) {
        ...
        window.location.href = '/login'; // Redireciona de volta para o dashboard após o cadastro
    } else {
        alert('Erro ao cadastrar usuario ou usuario já possui cadastro');
    }
});
